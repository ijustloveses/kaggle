...
for label in label_cols:
    ...
    file_path = "bilstm.best.{}.hdf5".format(label)
    checkpoint = ModelCheckpoint(file_path, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
    early = EarlyStopping(monitor='val_loss', mode='min', patience=3)

    pos = int(len(train) * 0.75)
    x_train = train_doc[0:pos]
    y_train = train[label].values[0:pos]
    x_val = train_doc[pos:]
    y_val = train[label].values[pos:]
    model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs,
              validation_data=(x_val, y_val), callbacks=[checkpoint, early])

    model.load_weights(file_path)

    logits = pd.DataFrame(model.predict(x_val), columns=[label])[label]
    labels = pd.DataFrame(y_val, columns=[label])[label]
    ...
    # dump to csv
    submit = pd.concat([submit, pd.DataFrame(preds, columns=[label])], axis=1)

submit.to_csv('submission.cnnlstm.csv', index=False)
